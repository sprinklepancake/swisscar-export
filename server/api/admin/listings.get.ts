...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const { data: cars, error } = await supabase
      .from('cars')
      .select('id, title, description, make, model, year, price, status, listing_type, is_featured, created_at, seller_id, seller:users!seller_id(id, name, email)')
    ...
    return {
      ...
      listings: (cars || []).map((c: any) => ({
        id: c.id, title: c.title, description: c.description, make: c.make, model: c.model, year: c.year, price: c.price,
