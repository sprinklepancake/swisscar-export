// server/api/cars/featured.get.ts - OPTIMIZED VERSION
...
export default defineEventHandler(async () => {
  try {
    ...
    // Only fetch necessary fields - removed seller data for initial load
    // Seller data can be fetched on-demand when viewing car details
    ...
    return {
      ...
      cars: (cars || []).map((car: any) => ({
        ...
        // Seller data removed - fetch on detail page only
      })),
    }
  } catch (error: any) {
